...
from bart_zero_shot_classification_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"architectures": ["BartForSequenceClassification"], "model_type": "bart"}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        BARTZeroShotClassificationPipeline.from_pretrained(
